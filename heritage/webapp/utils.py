...
def get_cards_and_tags_for_format(format, request):
  raise NotImplementedError

def get_cards_and_tags_for_format(format, request):
  raise NotImplementedError

# def get_texts_and_tags_for_format(format, request):
#   """Returns a dict with cards and tags corresponding to the format.
#
#   Args:
...
#   if tag_name:
#     texts = Card.objects.filter(format__exact=category).filter(tag__exact=tag_name).order_by("publication_date")
